...
| HG-18 | Integrations | AEBAS/portal/VC/e-mail contracts unknown (GAP-006, Q-010–Q-013) | Automation deferred; manual fallbacks needed | Backend layer + fallbacks | P2 | Interface-discovery workshop; import/manual-first phasing per SRS 11 | M-integrations |
...
| HG-12 | Manual-verify first; backend interface for future channel | FeePayment service + verify endpoint (verifier ≠ recorder) | Fee panel with matrix validation + dup-UTR warning | FeePayment + FeeMatrix tables | Matrix validation tests per system/level/slab; dup-UTR test | Channel confirmed later → backend swap | Fees recorded/verified for demo cases; mismatches flagged |
...
| HG-18 | Discovery workshop; backend's behind ports; manual fallback always | import endpoints with provenance (checksum, period, importer) | import screens + provenance display | import-log tables | fallback tests: statutory step completes with manual entry when backend down (NFR-080) | Contracts never materialize — manual mode is permanent fallback | Each integration demo-able in manual mode; backend swap documented |
