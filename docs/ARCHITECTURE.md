...
> The extraction + assessment pipeline is unchanged and acts on the _received_ report.
>
...
> consultant → engine → senior/board review → clarification (college) → hearings (President-appointed
...
| Consultant                  | `consultant`                  | Dealing staff; processes allotted colleges, drafts, submits            |
...
Consultants → consultant teams (Team-1 → Gaurav Bhandari; Team-2 → Kritika). College users bind to an
...
(allotted/assigned consultant, supervises submitter, college owner, hearing panel member).
...
uploaded → processing → processed → under_validation → senior_review → board_review
   │(err)                    ▲(return)          ▲(re-loop)     │
   └→ failed(retry)   clarification_open ─(college)→ clarification_responded ─(consultant review)→ clarification_reviewed
                             ▲(request_revision R1)──────────────────────────────────────────────────┘ │(submit)
board_review ─(board)→ request_hearing → hearing_requested ─(president)→ hearing_scheduled
                                                              (committee minutes)→ board_review
board_review ─(board)→ approved ─(secretariat dispatch, only if compliance=complied)→ closed   ·   reject → rejected → revise
```

The dealing-staff role was renamed `consultant` → `consultant` (display "Consultant"). Clarification now has a **Consultant review** sub-step (accept → submit, or request revision R1 → back to college); **Final-order dispatch is gated on `compliance=complied`**.
Ownership/scoping: consultant queue = cases whose `(system,state)` ∈ their `staff_allotments`; senior =
...
enforced by the state→role guard (a consultant can't decide; only the President appoints a committee;
...
penalty rows; the dealing consultant adds **manual** `monetary` (₹25-lakh ghost-faculty) +
...
parameters** vs the document-true fixture; hybrid = 34s, 15/20 pages (Docling _preprocess_
...
4. **cdm.stage** — builds the **Canonical Document Model** (`engines/extraction/cdm/`) and,
   ...
   _views_ of it, not the canonical format. Builder sub-stages (all generic, no NCISM
...
- **Extractors** (`extractors/`) are precision-first: a parameter is `found` (with provenance)
  or `missing` — never invented. Fields whose proforma labels sit next to MESAR _requirement_
...
user's roles by priority (`admin > president > board_member > senior_consultant > consultant
