...
function Map({
  ...
  const [prevStyle, setPrevStyle] = useState(targetStyle);

  if (targetStyle !== prevStyle) {
    setPrevStyle(targetStyle);
  ...
  const initialMapStyles = useRef(mapStyles);
  ...
  useEffect(() => {
    ...
    const initialStyle =
      initialTheme.current === "dark" ? initialMapStyles.current.dark : initialMapStyles.current.light;
    ...
    const styleDataHandler = () => {
      ...
      styleTimeoutRef.current = setTimeout(() => {
        ...
        if (initialProjection.current) {
          map.setProjection(initialProjection.current);
        }
      }, 100);
    };
...
function DefaultMarkerIcon() {
  return (
    <div
      className="relative h-4 w-4 rounded-full border-2 border-white bg-brand-accent shadow-lg" />
  );
}
...
function PopupCloseButton({
  ...
  return (
    <Button
      ...
      className="focus-visible:ring-ring hover:bg-muted text-foreground absolute top-1 right-1 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-inset p-0 shadow-none border-none">
...
function ControlButton({
  ...
  return (
    <Button
      ...
      variant="ghost"
      className={cn(
        "flex size-8 items-center justify-center transition-all p-0 shadow-none border-none min-w-0 rounded-none",
...
function MapRoute(
  ...
  const latestRef = useRef({ onClick, onMouseEnter, onMouseLeave });
  useEffect(() => {
    latestRef.current = { onClick, onMouseEnter, onMouseLeave };
  ...
  useEffect(() => {
    ...
    map.addLayer({
      ...
      paint: {
        ...
        ...(initialPaintRef.current.dashArray && { "line-dasharray": initialPaintRef.current.dashArray }),
      },
    });
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
      ...
      latestRef.current.onMouseEnter?.();
    };
    const handleMouseLeave = () => {
      map.getCanvas().style.cursor = "";
      latestRef.current.onMouseLeave?.();
    };
...
function MapArc(
  ...
  const initialConfigRef = useRef({ hitWidth, mergedLayout, mergedPaint });
  ...
  useEffect(() => {
    ...
    map.addLayer({
      ...
      paint: {
        ...
        "line-width": initialConfigRef.current.hitWidth,
        "line-opacity": 1,
      },
    }, beforeId);
    ...
    map.addLayer({
      ...
      type: "line",
      source: sourceId,
      layout: initialConfigRef.current.mergedLayout,
      paint: initialConfigRef.current.mergedPaint,
    }, beforeId);
  ...
  }, [isLoaded, map, sourceId, layerId, hitLayerId, beforeId]);
...
function MapClusterLayer(
  ...
  const { map, isLoaded } = useMap();
  const latestRef = useRef({ onClusterClick, onPointClick });
  useEffect(() => {
    latestRef.current = { onClusterClick, onPointClick };
  ...
  useEffect(() => {
    ...
    map.addSource(sourceId, {
      type: "geojson",
      data: initialConfigRef.current.data,
      cluster: true,
      clusterMaxZoom: initialConfigRef.current.clusterMaxZoom,
      clusterRadius: initialConfigRef.current.clusterRadius,
    });
    ...
    map.addLayer({
      ...
      paint: {
        "circle-color": [
          "step",
          ["get", "point_count"],
          initialConfigRef.current.clusterColors[0],
          initialConfigRef.current.clusterThresholds[0],
          initialConfigRef.current.clusterColors[1],
          initialConfigRef.current.clusterThresholds[1],
          initialConfigRef.current.clusterColors[2],
        ],
        "circle-radius": [
          "step",
          ["get", "point_count"],
    ...
    map.addLayer({
      ...
      filter: ["!", ["has", "point_count"]],
      paint: {
        "circle-color": initialConfigRef.current.pointColor,
        "circle-radius": 5,
  ...
  }, [isLoaded, map, sourceId, clusterLayerId, clusterCountLayerId, unclusteredLayerId]);
  ...
  useEffect(() => {
    ...
    const handleClusterClick = async (
      ...
      if (latestRef.current.onClusterClick) {
        latestRef.current.onClusterClick(clusterId, coordinates, pointCount);
      } else {
    ...
    const handlePointClick = (
      ...
      if (!latestRef.current.onPointClick || !e.features?.length) return;
      ...
      latestRef.current.onPointClick(feature, coordinates);
    };
    ...
    const handleMouseEnterPoint = () => {
      if (latestRef.current.onPointClick) {
        map.getCanvas().style.cursor = "pointer";
      }
    };
