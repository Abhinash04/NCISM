...
function ReportDropzone({ file, onSelect, onClear }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-xs text-muted-foreground">PDF only · a single Regulatory/Assessment Report</span>
    </div>
  );
}
...
export function ApplicationUpload() {
  ...
  // Case details (session, intake, permission type, visitation dates) are NOT
  // entered here — they are extracted from the uploaded Regulatory Report during
  // processing and used to populate letters/orders. The visitor only selects the
  // institution and uploads the report.
  const onSubmit = (e) => {
    e.preventDefault();
    if (!canSubmit) return;
    mutate({ institutionId, file }, {
  ...
  return (
    <div className="p-5 md:p-8 space-y-6 max-w-2xl mx-auto">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              Choose the institution and upload its Regulatory/Assessment Report. Case details are extracted from the report during processing — no manual entry needed.
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <Card>
          <CardContent className="p-5 space-y-3">
            <Label className="text-sm font-semibold">Regulatory/Assessment Report (PDF) <Req /></Label>
