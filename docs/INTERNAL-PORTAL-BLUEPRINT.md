...
1. **Role model expanded to 13.** The blueprint's 4 roles were kept as legacy, but the real NCISM
   ...
   `senior_consultant`, `consultant`, `visitor`, `college`, `hearing_committee`,
...
4. **State machine names differ** from §3 (uploaded→processing→processed→under_validation→
   ...
   maker-checker is the **org chain** (consultant→senior→board), not a flat Analyst→Reviewer.
...
> async worker; RBAC-matrix + E2E; MFA + code-splitting). **Phase 7a–7c** added UG Unani + UG
> Sowa-Rigpa + PG Ayurveda/Unani/Siddha rulesets (six authored + active). Remaining: Phase 7 UG Siddha
> content + non-Ayurveda/PG extractor tuning, then notifications (8) + production readiness (9) + reports depth (10).
...
| **Reviewer** _(Approver)_          | R2 Board Member + R3 President     | The **checker** — reviews validated assessments, **approves/rejects**, archives. Read-all.                                                               |
| **Analyst** _(Uploader/Validator)_ | R1 Dealing Staff + validation work | The **maker** — uploads reports, runs processing, validates extracted data, resolves extraction issues, submits for review.                              |
| **Viewer** _(Auditor)_             | R9 Commission Observer             | Read-only across assessments, reports/analytics, audit.                                                                                                  |
...
(`approvedBy ≠ submittedBy`)**, never by removing a role — so that senior can review _others'_
...
**Permission matrix** (C create · R read · U update · A approve/transition · _own_ = own-scope only):

| Resource              | Administrator                             | Reviewer                             | Analyst                                        | Viewer |
| --------------------- | ----------------------------------------- | ------------------------------------ | ---------------------------------------------- | ------ |
| Institutions (master) | C R U D                                   | R                                    | R                                              | R      |
| Assessments           | R                                         | R (all) · A (approve/reject/archive) | C · R · U _(own, pre-approval)_ · A _(submit)_ | R      |
| Validation issues     | R                                         | R                                    | C · R · U _(resolve, own)_                     | R      |
| Users & roles         | C R U D                                   | –                                    | –                                              | –      |
| Rulesets / standards  | C R U D _(draft)_ · A _(activate w/ ref)_ | R                                    | R                                              | R      |
| Reports / analytics   | R                                         | R                                    | R _(own + all)_                                | R      |
| Audit log             | R                                         | R                                    | R _(own actions)_                              | R      |
...
| **Validation** | Analyst _(owner)_         | confirm fields, resolve issues, **submit** → **Review**                 | all blocking issues resolved               |
| **Review**     | Reviewer _(≠ owner)_      | **approve** → **Approved** · **reject** → **Rejected**                  | maker-checker guard                        |
...
- **Dashboard:** _My uploads_, _Needs validation (mine)_, _Rejected back to me_, _Failed jobs_,
  throughput tiles.
- **Nav:** Dashboard · Uploads · Assessments · Institutions _(read)_ · Reports · Settings.
- **Pages / forms / tables:**
  - _Upload_ — drag-drop form (PDF) + institution picker only. Case details (session, intake, permission type, visitation dates) are **not entered by the visitor** — they are extracted from the report during processing.
  - _Upload queue_ — table (file, institution, state, progress, uploaded-at, actions).
  - _Assessment list_ — table with filters (state, institution, session, date), bulk select.
  - _Assessment detail_ — tabs: **PDF · Structured view · Extracted JSON · Validation · History**.
  - _Validation workbench_ — field-by-field confirm, extraction **issue list** (severity, location),
...
- **Dashboard:** _Awaiting my review_, _Recently approved_, _Institution watchlist_, aging/deadline
  tiles.
...
- **Nav:** Assessments _(read)_ · Institutions _(read)_ · Reports · Audit _(read)_.
...
| **7 — Non-Ayurveda rulesets**       | 🟡 In progress | Coverage                                            | **7a** UG Unani + **7b** UG Sowa-Rigpa + **7c** PG Ayurveda/Unani/Siddha authored from `markdown/MESAR_*.md` (rules + punitive + report-template module + synthetic golden fixture) and **active** (six rulesets). PG uses a flat-standards model (no engine changes). Remaining: UG Siddha content + tuning the parameter extractors to real non-Ayurveda/PG report layouts. |
...
**Domain:** `institutions` (institute*id `AYU/UNI/SID/SWR####`, system, state, name, …) ·
**`staff_allotments`** (user_id × system × state).
**Cases (built):** `applications` (+`outcome`, `approved_seats`, `intake`, `level`, `permission_type`,
`visitation*\*`) · `application_events`(timeline) ·`clarifications`·`hearings`+`hearing_members`·`board_meetings`+`board_meeting_items`·`letters`(generated Clarification/Hearing/Final Order).
...
hash, bytes),`assessment_report`(canonical parsed JSON — JSONB; immutable once Approved),`validation_issues`(assessment_id, field_path, severity, status, resolution, resolved_by),`assessment_events`(from_state, to_state, actor_id, at, note).
**Governance:**`audit_log`(append-only: actor, action, entity, entity_id, before, after, at, ip),`ruleset_versions`(system, level, version, effective_session, payload, activated_by, ref, status),`report_definitions` (optional saved reports).
...
| `validation.service`                            | 🔜 Planned | structured extraction-issue tracking (today: consultant re-runs/edits inline)                                                                  |
